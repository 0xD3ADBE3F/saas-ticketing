...
/**
 * GET /api/organizations/[id]/settlements/[settlementId]/payments
 * Get payments in a specific settlement
 */
export async function GET(
  ...
  // Only ADMIN and FINANCE roles can view settlement payments
  ...
  try {
    const result = await mollieSettlementService.getSettlementPayments(
      organizationId,
      settlementId,
      { from, limit }
    );
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to get settlement payments" },
      { status: 500 }
    );
  }
}
